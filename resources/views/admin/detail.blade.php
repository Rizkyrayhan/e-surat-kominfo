...
<div class="row g-4">
        ...
        <div class="stat-card p-4 p-lg-5 mb-4">
            <div class="d-flex justify-content-between align-items-start border-bottom pb-4 mb-4">
                <div>
                    ...
                    <p class="text-muted small mb-0">Nomor Agenda: SK - {{ date('Y') }} - {{ str_pad($surat->id, 3, '0', STR_PAD_LEFT) }}</p>
                </div>
                <div>
    ...
    <div class="col-lg-4">
        ...
        <div class="stat-card p-4 mb-4 border-primary">
            ...
            <form action="{{ route('admin.surat.update-status', $surat->id) }}" method="POST">
                @csrf
                @method('PATCH')
                <div class="mb-3">
                    <label class="form-label small fw-medium text-muted">Ubah Status Dokumen</label>
                    <select name="status" class="form-select border-primary">
                        <option value="pending" {{ $surat->status == 'pending' ? 'selected' : '' }}>Pending (Menunggu)</option>
                        <option value="diproses" {{ $surat->status == 'diproses' ? 'selected' : '' }}>Diproses (Verifikasi)</option>
                        <option value="dikirim" {{ $surat->status == 'dikirim' ? 'selected' : '' }}>Dikirim (Diteruskan)</option>
                        <option value="selesai" {{ $surat->status == 'selesai' ? 'selected' : '' }}>Selesai (Arsip)</option>
                    </select>
                </div>
                <button type="submit" class="btn btn-primary w-100 rounded-pill"><i class="bi bi-save me-2"></i> Simpan Perubahan</button>
            </form>
            <div class="mt-3 pt-3 border-top text-center">
                <a href="{{ route('admin.surat.print', $surat->id) }}" target="_blank" class="btn btn-outline-secondary w-100 rounded-pill"><i class="bi bi-printer me-2"></i> Cetak Lembar Disposisi</a>
            </div>
        </div>
